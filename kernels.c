...
void bokeh_pix(int dim, pixel *src, short *flt, pixel *dst) {
    ...
    for (i = 1; i < dim_mn1; i++) {
        ...
        for (j = 1; j < dim_mn1; j++) {
            if (!flt[rij]) {
                PACKEDPIX pp1, pp2, pp3, pp4, pp5, pp6, pp7, pp8, pp9;

                PIXEL_TO_PACKEDPIX(pp1, src[rij])
                PIXEL_TO_PACKEDPIX(pp2, src[rij-1])
                PIXEL_TO_PACKEDPIX(pp3, src[rij+1])
                PIXEL_TO_PACKEDPIX(pp4, src[rij-dim])
                PIXEL_TO_PACKEDPIX(pp5, src[rij+dim])
                PIXEL_TO_PACKEDPIX(pp6, src[rij-dim_ps1])
                PIXEL_TO_PACKEDPIX(pp7, src[rij-dim_mn1])
                PIXEL_TO_PACKEDPIX(pp8, src[rij+dim_ps1])
                PIXEL_TO_PACKEDPIX(pp9, src[rij+dim_mn1])

                pp1 += pp2 + pp3 + pp4 + pp5 + pp6 + pp7 + pp8 + pp9;
                pp1 /= 9;
                PACKEDPIX_TO_PIXEL(pp1, dst[rij])
